...
static int	gpio_dtr = -1;
static int	gpio_dcd = -1;
static int	gpio_dsr = -1;
...
static int gpio_sync_ri(void)
{
	...
	if ((gpio_ri >= 0) && (desc = gpio_to_desc(gpio_ri))) {
		...
		owned_by_app_proc = desc->owned_by_app_proc;
                if ((gpio_dtr >= 0) && (desc = gpio_to_desc(gpio_dtr)))
			desc->owned_by_app_proc = owned_by_app_proc;
                if ((gpio_dcd >= 0) && (desc = gpio_to_desc(gpio_dcd)))
			desc->owned_by_app_proc = owned_by_app_proc;
                if ((gpio_dsr >= 0) && (desc = gpio_to_desc(gpio_dsr)))
			desc->owned_by_app_proc = owned_by_app_proc;
...
static int sierra_gpio_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, DT_COMPATIBLE);
	if (!np) {
		dev_err(&pdev->dev, "Cannot find device tree node compatible \"%s\"\n",
			DT_COMPATIBLE);
		return -ENOENT;
	}
	...
	for_each_property_of_node(np, pp) {
		...
		if (gpio >= 0) {
			...
			if (test_bit(FLAG_DTR, &desc->flags) && gpio_dtr == -1) {
				gpio_dtr = gpio_alias_map[ngpios].gpio_num;
				gpio_sync_ri();
			}
			if (test_bit(FLAG_DCD, &desc->flags) && gpio_dcd == -1) {
				gpio_dcd = gpio_alias_map[ngpios].gpio_num;
				gpio_sync_ri();
			}
			if (test_bit(FLAG_DSR, &desc->flags) && gpio_dsr == -1) {
				gpio_dsr = gpio_alias_map[ngpios].gpio_num;
				gpio_sync_ri();
			}
